...
library(MASS)
crmodel <- lm( NUM_CRIMES~GRAD_YEAR+ZIP+COHORT_SIZE,data=edu_crime)
summary(crmodel)
...
```
```{r}
#Adding violent category
crime <- data.table(crime)
crime.cat <- crime[,c("CATEGORY","YEAR","ZIP_CHAR")]
crime.cat$VIOLENT <- 0
crime.cat[CATEGORY %in% c("ASSAULT","MURDER/INFORMATION","AGGRAVATED ASSAULT","HOMICIDE","SEX OFFENSES","ROBBERY")]$VIOLENT <- 1
crime.cat$NUM_CRIMES <- 1
crime.cat$CATEGORY <- NULL

simple.crime=aggregate(crime.cat,by=list(crime.cat$YEAR, crime.cat$ZIP_CHAR), FUN=sum)

drops = c("YEAR","ZIP_CHAR")
simple.crime <- simple.crime[,!names(simple.crime) %in% drops]
colnames(simple.crime) = c("YEAR", "ZIP", "VIOLENT","NUM_CRIMES")

edu_crime <- merge(edu.grad, simple.crime,by.x=c("GRAD_YEAR","ZIP"),by.y=c("YEAR","ZIP"),all.x = TRUE)

edu_crime$NUM_CRIMES[is.na(edu_crime$NUM_CRIMES)] <- 0
edu_crime$VIOLENT[is.na(ed_crime$VIOLENT)] <- 0
...
educast <- dcast(edu_crime, GRAD_YEAR~ZIP,value.var = "DROP_RATE")


edu_crime2009 <- edu_crime[GRAD_YEAR==2009]
plot(edu_crime2009$NUM_CRIMES, edu_crime2009$DROP_RATE)
crmodel <- lm(VIOLENT~NUM_COHORT+DROP_RATE+ZIP)
